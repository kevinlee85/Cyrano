...
public class TaskSearchForTriggers extends
	...
	protected JSONObject doInBackground(List<BluetoothDevice>... arg0) {
		...
		String accessToken = SharedpreferenceUtility.getInstance(activity)
				.getString(Constants.ACCESS_TOKEN);
		String userId = SharedpreferenceUtility.getInstance(activity)
				.getString(Constants.CYRANO_USER_ID);

		StringBuilder addresses = new StringBuilder("");
		/*
		 * for (int i = 0; i < devices.size(); i++) { addresses.append("'" +
		 * devices.get(i).getAddress() + "'"); if(i != devices.size() - 1) {
		 * addresses.append(","); }
		 */
		...
		for (int i = 0; i < DataStore.getInstance().getbluletoothmac().size(); i++) {
			...
			if (DataStore.getInstance().getIDSofBTIDS().get(Purevalues) != null
					&& DataStore.getInstance().getIDSofBTIDS().get(Purevalues) > 1)
			{
				Log.i(TAG, Purevalues + "SKIP");
				addresses.append("'" +"1:1"+ "'");
			}
				else
		...
		String requestUrl = Constants.SERVER_ROOT + Constants.SEPERATOR
				+ Constants.SERVICE_SEARCH_FOR_TRIGGERS + Constants.SEPERATOR
				+ accessToken + Constants.SEPERATOR + userId
				+ Constants.SEPERATOR + addresses.toString();

		// http://cyrano.cjcornell.com/REST/index.php/TriggerListMatchedBT/{access_token}
		// / {user_id} / { List of comma seperated single quoted BT mac address}

		 Log.d(TAG, "Sending GET request at URL " + requestUrl);

		try {
			HttpGet httpGet = new HttpGet(requestUrl);
			JSONObject response = new JSONObject(httpClient.execute(httpGet,
	...
	/**
	 * This is executed after doInBackground method completes. It will set the
	 * default settings based on the contents of the JSONObject returned by the
	...
	protected void onPostExecute(JSONObject response) {
		...
		// If there was a problem, the JSONArray is null, and we should simply
		// return
		if (response == null) {
