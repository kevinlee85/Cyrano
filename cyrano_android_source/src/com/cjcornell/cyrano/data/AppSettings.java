...
import com.cjcornell.cyrano.task.TaskGetDefaultSettings;
...
public class AppSettings {
	...
	public static Boolean terseMode, tsAudio, graphicalMode, friendFinder,
			friendAudio, autoDisplayFriends;
	...
	/**
	 * Initialize the settings based on the settings file. If there are issues
	 * with the file, it will set all variables to a local default value, then
	 * attempt to load the global defaults from the database.
	 */
	public static void initSettings(ActivityCyrano activity) {
		...
		/**
		 * If there was an error, two things must happen 1. Set all variables to
		 * local defaults - this ensures that the settings at least have SOME
		 * default value, should the DefaultSettingsTask not complete before a
		 * setting is needed. 2. Start the DefaultSettingsTask to grab all the
		 * global defaults from the database
		 */
		if (errorFlag) {
			Log.v(TAG, "Error on getting local settings - grabbing defaults");
			setDefaults();
			new TaskGetDefaultSettings(activity).execute();
		}
	}
	...
	public static void saveSettings(Context context) {
		try {
			FileOutputStream fos = context.openFileOutput(SETTINGS_FILE,
	...
	/**
	 * Set the default values based on given parameters - this is meant to be
	 * used with the results of a database call.
	 */
	public static void setDefaults(String terseModeStr, String tsAudioStr,
			String graphicalModeStr, String friendFinderStr,
			String friendAudioStr, String textSizeStr, String gpsTimeDelayStr,
			String maxFriendsStr, String autoDisplayFriendsStr) {

		double gpsTimeDelayDouble;
		...
		try {
			...
			textSize = Integer.parseInt("12");
		}

		try {

			gpsTimeDelayDouble = Double.parseDouble(gpsTimeDelayStr);
		}

		catch (Exception e) {

			gpsTimeDelayDouble = 1.02;

		}
		if (gpsTimeDelayDouble < 0.5) {
			gpsTimeDelay = 0.5;
		} else {
			gpsTimeDelay = gpsTimeDelayDouble;
		}

		try {
			maxFriends = Integer.parseInt(maxFriendsStr);
		} catch (Exception e) {
			maxFriends = Integer.parseInt("3");
		}
	...
	/**
	 * Get the text size value for descriptive text, based on the current
	 * settings
	 */
